...
namespace AForge.Math
{
	...
	public static class FourierTransform
	{
        ...
        public static Complex[] ApplyHannWindow(Complex[] data, int starter, int N)
        {
            double p;
            for (int n = 0; n < N; n++)
            {
                p = data[n + starter].Re;
                data[n + starter].Re *= 0.5f * (1 - (float)Math.Cos((2 * Math.PI * n) / (N - 1)));
                data[n + starter].Re += p;
            }
            return data;
        }

        public static Complex[] convertTOdB(Complex[] data)
        {
            double maxx = 0;
            for (int i = 0; i < data.Length; i++)
                if (data[i].Re > maxx)
                    maxx = data[i].Re;
            for (int i = 0; i < data.Length; i++)
                data[i].Re = 10.0 * Math.Log10(Math.Abs(data[i].Re)/(double)maxx) - maxx;
            return data;
        }
